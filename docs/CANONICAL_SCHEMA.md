# Canonical Data Schema (`TrajectorySample`)

To ensure the framework remains decoupled from any specific dataset (real or synthetic), every dataset adapter must convert its native data into this strict, canonical `TrajectorySample` Pydantic schema. 

Once converted, the downstream tokenizers, continuous coordinate pipelines, and models will **only** interact with this schema.

## Proposed Pydantic Schema Design
...
1. **Synthetic Tracking:** `is_synthetic` must be strictly enforced. The training pipeline will sum the `is_synthetic` flags in a batch or epoch. If `sum(is_synthetic) > (len(batch) / 2)` during production training, the pipeline will throw a `SyntheticDataDominanceWarning`.
2. **Metadata Integrity:** Even if a real dataset lacks `pressure` or `timestamp` (which many do), the schema supports them as `Optional[float]` to future-proof the architecture.
3. **Decoupling:** Models will extract `(x, y, pen_state)` from `Stroke.points`, and `text` from `TrajectorySample.text`. They will not care if the data came from LipiTK, HP Labs, or a `SyntheticTrajectoryGenerator`.
